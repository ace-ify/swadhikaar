...
- **WS3 — Build PS-01** (SIG→time-grid parser → editable verification → pictogram schedule → plain-language plan → voice readback on existing Murf TTS → reminders on existing `voice_calls` scheduler → caregiver share; strip diagnostic notes; interactions advisory), **then PS-02** (real empanelled-hospital data → location+specialty finder → document checklist → visit steps → plain-language guidance → bonus bounded RAG; reuse facilities/maps/voice/ASHA).
